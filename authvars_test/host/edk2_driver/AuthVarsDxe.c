...
)
{
  ...
  LOG_INFO("- %S = %s", EFI_SETUP_MODE_NAME, mSetupModeNames[SetupMode]);
  ...
  LOG_INFO("- %S = %s", EFI_CUSTOM_MODE_NAME, mCustomModeNames[Data]);
...
)
{
  ...
  */
  ...
  if (EFI_ERROR(Status)) {
    LOG_ERROR("TaOpenSession() failed. (Status=%x)", Status);
    goto Exit;
  }
